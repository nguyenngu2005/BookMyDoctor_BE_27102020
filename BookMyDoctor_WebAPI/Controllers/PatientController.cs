...
namespace BookMyDoctor_WebAPI.Controllers
{
    ...
    public class PatientsController : ControllerBase
    {
            ...
            CancellationToken ct)
        {
            try
            {
                var result = await _service.GetAllPatientsAsync(name, appointDate, status, ct);
            ...
            {
                _logger.LogError(ex.Message, "Lỗi khi lấy danh sách bệnh nhân");
        ...
        [Authorize(Roles = "R02")]
        public async Task<IActionResult> UpdatePatient(int id, [FromBody] PatientDetailRequest dto, CancellationToken ct)
        {
            if (dto == null)
            ...
            try
            {
                var result = await _service.UpdatePatientAsync(id, dto, ct);
                if (!result.Success)
