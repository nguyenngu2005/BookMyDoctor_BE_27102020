...
public sealed partial class BookingService : IBookingService
{
    ...
    public async Task<BookingResult> PublicBookAsync(PublicBookingRequest req, CancellationToken ct)
    {
        ...
        await SendBookingEmailAsync(
            ...
            department: string.IsNullOrWhiteSpace(req.Department) ? "Khám tổng quát" : req.Department!,
            date: req.Date,
            ...
            if (!string.IsNullOrWhiteSpace(toEmail))
            {
                ...
                var mail = new System.Net.Mail.MailMessage
                {
                    ...
                    Body =
                        ...
                        $"<li>Ngày khám: <b><span style='color:red;'>{date:dd/MM/yyyy}</span></b></li>" +
                        $"<li>Giờ hẹn: <b><span style='color:red;'>{hour:HH\\:mm}</span></b></li>" +
